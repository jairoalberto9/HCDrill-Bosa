/*
* SocksHTTP module
* Description: This covers all encryption scheme related to SocksHTTP
* Author: PANCHO7532
* Version: 1.0
*/
...
function parseDecoded(data) {
    ...
    var json = {};
    for(c = 0; c < parsedContent.length; c++) {
        switch(parsedContent[c]["@_key"]) {
            ...
            case "proxyRemotoPorta":
                ...
                break;
            case "file.msg":
                json["note1"] = parsedContent[c]["#text"];
                break;
            case "proxyPayload":
                json["payload"] = parsedContent[c]["#text"];
                break;
            case "udpForward":
            ...
            case "file.validade":
                ...
                break;
            case "hostSniSSL":
                json["sslsni"] = parsedContent[c]["#text"];
                break;
        }
    }
    ...
    } else if(!!json["proxyURL"] && !!json["proxyAddress"]){
        json["proxyURL"] = json["proxyAddress"] + ":" + json["proxyPort"];
    }
    json["raw"] = data;
    return JSON.stringify(json);
}
module.exports.decrypt = function(filecontent) {
    ...
    try {
        keyFile = JSON.parse(fs.readFileSync("./lib/keyFile.json").toString())["socksHTTP"];
    } catch(error) {
