...
function aesDecrypt(data, key) {
    ...
    var result = aesoutp1.update(data, "base64", "utf8");
...
function decryptStage(fileContent, configFile) {
    ...
    response["error"] = 0;
    try {
        keyFile = JSON.parse(fs.readFileSync(configFile["keyFile"]).toString())["httpCustom"][0];
    } catch(error) {
    ...
    for(let c = 0; c < keyFile.length; c++) {
        ...
        try {
            ...
            if(response["content"].length > 1) {
                if(response["content"].indexOf("[splitConfig]") != -1) { complete = true };
                break;
            } else {
                throw "False UTF-8";
            }
...
function decryptStage3(fileContent, configFile) {
    ...
    try {
        keyFile2 = JSON.parse(fs.readFileSync(configFile["keyFile"]).toString())["httpCustom"][1];
    } catch(error) {
        response["error"] = error;
        return response;
    }
